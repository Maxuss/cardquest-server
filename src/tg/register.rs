...
pub fn  schema() -> UpdateHandler<anyhow::Error> {
    ...
    let message_handler = Update::filter_message()
        ...
        .branch(case![DialogueState::GetUsername { id, card_hash }].endpoint(receive_username_text));

    let callback_query_handler = Update::filter_callback_query()
        .branch(case![DialogueState::GetUsername { id, card_hash }].endpoint(receive_username_callback));
...
async fn receive_username_callback(
    ...
    if let Some(username) = &q.data {
        ...
        finish_registration(bot, dialogue.chat_id(), pool, username.to_owned(), id, card_hash).await?;
...
pub async fn register(bot: AutoSend<Bot>, msg: Message, dialogue: SignupDialogue, token: String, pool: PgPool) -> anyhow::Result<()> {
    if token.len() != 8 {
        bot.send_message(msg.chat.id, "Неверный токен регистрации!").await?;
        return Ok(())
    }
    let stage = if let Some(stage) = sqlx::query_as::<_, UserRegStage>("SELECT * FROM users_reg WHERE starts_with(hash, $1)").bind(token).fetch_optional(&pool).await? {
        stage
    } else {
        bot.send_message(msg.chat.id, "Неверный токен для регистрации!").await?;
        return Ok(())
    };

    bot.send_message(msg.chat.id, "Вы начинаете регистрацию на квест.").await?;

    let deleted = sqlx::query("DELETE FROM users_reg WHERE starts_with(hash, $1)").bind(&stage.hash).execute(&pool).await?;

    if deleted.rows_affected() != 1 {
        log::warn!("Invalid amount of rows affected for delete operation, expected 1 but got {}", deleted.rows_affected())
    }

    bot.send_message(msg.chat.id, "Введите предпочитаемый ник.").reply_markup(make_username_keyboard(&msg)).await?;

    dialogue.update(DialogueState::GetUsername { id: stage.id, card_hash: stage.hash }).await?;

    Ok(())
}

async fn user_exists(pool: &PgPool, username: &String) -> anyhow::Result<bool> {
    if let Some(_) = sqlx::query("SELECT * FROM users WHERE username = $1").bind(username).fetch_optional(pool).await? {
        Ok(true)
    } else {
        Ok(false)
    }
...
async fn finish_registration(bot: AutoSend<Bot>, id: ChatId, pool: PgPool, username: String, uuid: Uuid, card_hash: String) -> anyhow::Result<()> {
    let rows = sqlx::query("INSERT INTO users VALUES($1, $2, $3)").bind(card_hash).bind(uuid).bind(username).execute(&pool).await?;
    if rows.rows_affected() < 1 {
        bot.send_message(id, "Не удалось провести регистрацию!").await?;
        return Ok(())
    }
